...
static int clip_analog(int cliptemp);
...
static const int reload_table[4] = { 0, 2, 4, 6 }; //sample count reload for 5220c and cd2501ecd only; 5200 and 5220 always reload with 0; keep in mind this is loaded on IP=0 PC=12 subcycle=1 so it immediately will increment after one sample, effectively being 1,3,5,7 as in the comments above.
...
void tms5220_device::process(int *buffer, unsigned int size)
{
	int buf_count=0;
	int i, bitout, zpar;
	int this_sample;
...
int tms5220_device::lattice_filter()
{
	// Lattice filter here
...
void tms5220_device::update_stream()
{
    switch (m_rs_ws)
    {
    case 0x02:
...
{
	int new_val;
	printf("rsq_w called %d - new state %d - new val %d\n", state, state & 0x01, (m_rs_ws & 0x01) | (state<<1));
...
{
	int new_val;
	printf("wsq_w called %d - new state %d - new val %d\n", state, state & 0x01, (m_rs_ws & 0x02) | (state<<0));
...
void tms5220_device::data_w(int data)
{
	printf("data_w called %x\n", data);
...
int tms5220_device::status_r()
{
 	printf("status_r called %x", m_rs_ws);
...
//{
//	int sample_data[MAX_SAMPLE_CHUNK];
